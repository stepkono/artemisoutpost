...
#include "MoonScannedAreaManager.h"
...
void UMoonScannedAreaManager::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	
	DOREPLIFETIME(UMoonScannedAreaManager, AreaScans);
}

// Sets default values for this component's properties
UMoonScannedAreaManager::UMoonScannedAreaManager()
{
...
void UMoonScannedAreaManager::BeginPlay()
{
	Super::BeginPlay();
	...
	if (bClientCtx)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("[MoonDataManager] MoonTexturer on AR_TILESET: %s"), MoonTexturer ? TEXT("FOUND") : TEXT("NULL"));
	}
	else
	{
...
// Called every frame
void UMoonScannedAreaManager::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UMoonScannedAreaManager::AddNewAreaScan(FAreaScan AreaScan)
{
	// Server-authoritative. The append replicates AreaScans -> clients run OnRep_UpdateForOfWar.
	AreaScans.Add(AreaScan);
}

bool UMoonScannedAreaManager::IsPositionExploredARMoon(FVector& GeoPosition, bool bForARMooon)
{
	if (!GeoRefsManager)
	{
...
void UMoonScannedAreaManager::OnRep_UpdateForOfWar()
{
...
void UMoonScannedAreaManager::UpdateFogOfWarTexture()
{
