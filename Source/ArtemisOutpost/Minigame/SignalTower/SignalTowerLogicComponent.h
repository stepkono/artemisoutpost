...
// Habitat (§8.7). Earth is a random bearing every time (never memorizable). Habitat would
// derive from the chosen habitat's geodetic bearing, but there is no habitat data model yet,
// so it is a random placeholder for now.
UCLASS(ClassGroup = (Minigame), meta = (BlueprintSpawnableComponent))
class ARTEMISOUTPOST_API USignalTowerLogicComponent : public UCoupledAxisLogicComponent
{
	GENERATED_BODY()
...
protected:
	...
	virtual void InitAxisTargets() override;
